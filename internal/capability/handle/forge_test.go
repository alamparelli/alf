...
func TestForge_MintAndValidate(t *testing.T) {
	...
	inst, err := ForgeInstance(tok, context.Background(), capability.ID("cap-ok"), Grants{})
	if err != nil {
...
func TestForge_ZeroTokenRejected(t *testing.T) {
	...
	inst, err := ForgeInstance(RuntimeToken{}, context.Background(), "cap", Grants{})
...
func TestForge_TamperedTokenRejected(t *testing.T) {
	...
	inst, err := ForgeInstance(fake, context.Background(), "cap", Grants{})
	if !errors.Is(err, ErrInvalidRuntimeToken) {
...
func TestForge_RejectionLeavesMintStateIntact(t *testing.T) {
	...
	// token comparison.
	_, _ = ForgeInstance(RuntimeToken{}, context.Background(), "cap-bad", Grants{})

	inst, err := ForgeInstance(tok, context.Background(), "cap-good", Grants{})
	if err != nil {
