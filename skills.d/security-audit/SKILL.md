...
**Audit priority** — under the §4.1 lockdown (post-#420) flat scripts in `~/data/tools/` are no longer invoked by the daemon at all; legacy entries there only land in the toolbox as "ignored" log lines, so they're not an active attack surface but still worth flagging for cleanup. WASM-kind bundles live at `~/data/tools/<id>/` (kind=wasm-tool) and `~/data/apps/<id>/` (kind=wasm-app), isolated by wazero + signed envelope (Layer 1 inner ring + Layer 2 of the 0.8.0 model). Their manifest-declared permissions cap blast radius, so they are lower priority unless the manifest itself looks suspicious — over-broad `fs.*` paths, broad `[[http.scopes]]` hosts, missing kind, or attempts to slip through 0.9.0-deferred blocks (`exec`, `secrets`). The legacy `~/data/skills.d/wasm/<id>/` path is migrated to the kind-keyed root by `migrateLegacyBundles` at boot and should not contain bundles in a healthy install. Prompt-based skills (`~/data/skills/*/SKILL.md`) are not executable but can carry prompt-injection payloads.
