...
- **POSIX file-mode hygiene** — every artefact under `<dataDir>/` carries a mode aligned with its trust domain (admin keys 0o600 / IPC sockets 0o660 / public artefacts 0o644). Defense-in-depth below AppArmor: both express "what the daemon process can touch" but at different levels (LSM hook vs inode). Full categorisation in [`docs/POSIX-PERMISSIONS-AUDIT.md`](POSIX-PERMISSIONS-AUDIT.md) (`#407`).
...
| `#384` marketplace bundle signing + TLS-pinned registry | L2 (post-#386) | **Daemon side shipped** on `release/0.8.0`: every marketplace install routes through `envelope.Verify` against the daemon's shared trust store via the new `internal/marketplace/bundle.go` (`readManifestFromZip` + `verifyBundle`); refuses unsigned, untrusted-key, tampered, or wrong-kind bundles before any file is written to disk. `Manager.SetTrustStore` wires the store; daemon boot calls it post-`setupWASMLoader` so every consumer (WASM loader, skills, marketplace, CRL refresher) shares one set of keys. `envelope.MarketplacePublicKey` (`go:embed marketplace_pubkey.minisign`) auto-adds the alf-marketplace publisher key to the trust store when a non-empty embed is present; dev builds log once and operators import third-party keys via `alf trust add`. `m.trusted[slug] = true` heuristic retired — `MarkTrusted` is now reserved for built-in apps. Legacy `installLegacy` / `downloadFile` / `downloadBinary` / `downloadSkills` / `downloadWebAsset` / `httpGet` deleted (~80 LoC dead code). HTTPS already enforced at construction time via `validateRegistryURL`. Wire contract: `bundle?arch=` for the ZIP, `bundle.sig?arch=` for the detached signature; ZIP must contain a top-level `manifest.toml` of `kind = marketplace-app` or `wasm-app`. **Server-side dependency**: the alf-marketplace repo (separate) needs a parallel upgrade to ship signed bundles; until then `Install`/`Update` surfaces `ErrBundleSignatureMissing` so operators see the upgrade requirement. Permission diff on Update widening shipped via #402 (see next row). 13 new tests. TLS cert pinning beyond system CA bundle deferred — the pubkey embed is the authority anchor, transport pinning would harden against CA mis-issuance only. |
| `#403` updater cosign verify + image digest pin | L2 (release-distribution) | **Shipped** on `release/0.8.0`: the auto-update Checker resolves every candidate tag's manifest digest via OCI HEAD (`Docker-Content-Digest`) and runs `cosign verify` against the alf-release identity (`https://token.actions.githubusercontent.com` / `^https://github.com/alamparelli/alf/\.github/workflows/release\.yml@`) BEFORE firing the operator notify. Verification failure aborts the notify, refusing to surface a tag that doesn't chain to the release pipeline. New `internal/platform/updater/cosign.go` carries `CosignVerifier{Binary,Issuer,IdentityRegex,Run}` with an injectable exec seam for tests; typed `ErrCosignBinaryNotFound` distinguishes "image lacks cosign" from "verify failed". `NotifyFunc` signature now includes the digest so the installer pulls `repo@<digest>` (defeats tag-repush attacks). Dockerfile adds cosign v2.4.1 (linux-amd64/arm64). Operator override: `ALF_DISABLE_COSIGN_VERIFY=1` (homelab dev), `ALF_COSIGN_ISSUER` / `ALF_COSIGN_IDENTITY_REGEX` for forks. 8 new tests across cosign + check-flow paths. |
| `#402` marketplace Update permission widening → ratification | L2 + admin boundary | **Shipped** on `release/0.8.0`: `Manager.Update` diffs old vs new manifest perms BEFORE committing. Widening enqueues a `KindPermissionWiden` item via the daemon-supplied `PermissionRatifier` callback (wired to `pending.DirStore.Append` in `cmd/alf-daemon/main.go`) and returns `ErrPermissionWideningPending` with the queue id. No-ratifier path returns `ErrPermissionWideningRefused` (no fallback to silent widening). Narrowing proceeds silently. New `internal/marketplace/permdiff.go` carries `diffPermissions` (set-difference, sort-stable, never-nil), the callback type, and the two error sentinels. New `readManifestJSONFromZip` helper in `bundle.go` reads the new manifest's perms from the in-memory ZIP without touching disk. `downloadAndExtractBundle` split into `downloadAndVerifyBundle` + `extractVerifiedBundle` so Update can interpose the gate between verify and extract. Acceptance verified: `TestUpdate_WideningWithRatifierEnqueues` asserts m.perms[slug] AND on-disk manifest.json are unchanged after a refused-pending Update — the running app keeps its old version until the operator runs `alf ratify <id>`. 12 new tests. |
...
| `#86` AppArmor + seccomp + CAP_SYS_ADMIN | L1 outer | **Shipped** on `release/0.8.0`: SEC-A02 — `CAP_SYS_ADMIN` + `CAP_SYS_CHROOT` dropped from `cap_add` in `internal/cli/templates/docker-compose.yml.tmpl` (code-search verified zero remaining callers of `syscall.Mount`/`Chroot`/`Unshare`/`PivotRoot` post-#406). SEC-A01 — custom AppArmor profile authored at [`scripts/apparmor-alf.profile`](../scripts/apparmor-alf.profile) (narrow-syscall posture, denies mount / pivot_root / SYS_ADMIN / SYS_MODULE / SYS_RAWIO). SEC-A11 — seccomp profile at [`scripts/seccomp-alf.json`](../scripts/seccomp-alf.json) (Docker-default-derived allowlist + explicit denies). SEC-A03 — already shipped via #325 (`AppTokenStore` / HMAC-signed per-app tokens replaced Referer-based isolation; Referer check is now defense-in-depth). **Not flipped by default**: docker-compose still ships `apparmor=unconfined` + no seccomp profile until soak-testing confirms the full daemon boot doesn't trip a denied syscall. Operator activation: `sudo apparmor_parser -r -W scripts/apparmor-alf.profile` then flip the security_opt line. **Follow-ups deferred**: per-spawn CAP_SETUID/SETGID drop on the LLM subprocess (zero-caps in the LLM); `<dataDir>/trust/` 0o750 once LLM has separate gid (deferred from #407); chown call sites for non-root daemon transition. |
...
| `#407` POSIX file-permission audit | L1 outer | **Shipped** on `release/0.8.0`: full categorisation of every `os.Chmod` / `os.MkdirAll` / `os.WriteFile` mode against the layered architecture's three trust domains (admin / process-isolation / public-via-CC). Audit doc at [`docs/POSIX-PERMISSIONS-AUDIT.md`](POSIX-PERMISSIONS-AUDIT.md). §2.1 Layer 1 outer ring updated to reference it as defense-in-depth alongside AppArmor. **Findings**: zero violations of trust-domain boundaries; every 0o600/0o700 site is in admin domain, every 0o660 socket is process-isolation, every 0o644/0o664 artefact is public-via-CC. **Two follow-ups deferred to #86** (sibling, Layer 1 outer ring): tighten `<dataDir>/trust/` to 0o750 once the LLM gets its own gid, and update chown call sites for the non-root daemon transition. No code changes required by this audit — the existing modes already match the architecture. |
