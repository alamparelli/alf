...
type Manifest struct {
	...
	Runtime     string      `toml:"runtime"`     // "go-wasip1" for now
...
type Permissions struct {
	...
	HTTP    []string `toml:"http"`    // allowed hostnames for raw http fetch
	Memory  bool     `toml:"memory"`  // access to long-term memory store
	Events  bool     `toml:"events"`  // emit inter-capability events
}

// LoadManifest parses a manifest.toml file and returns a validated Manifest.
