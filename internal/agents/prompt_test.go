...
func TestBuildPromptCodexBlock(t *testing.T) {
	teams := []*TeamConfig{{Name: "t", Agents: []AgentConfig{{Name: "a", Tier: "default"}}}}

	// Codex backend should include the codex-specific block.
	prompt := BuildOrchestratorPrompt(teams, "", "codex")
	if !strings.Contains(prompt, "NEVER ASK QUESTIONS") {
		t.Error("codex backend should include action-bias block")
	}

	// Non-codex backend should not.
	prompt = BuildOrchestratorPrompt(teams, "", "cli")
	if strings.Contains(prompt, "NEVER ASK QUESTIONS") {
		t.Error("cli backend should not include codex block")
	}
}
