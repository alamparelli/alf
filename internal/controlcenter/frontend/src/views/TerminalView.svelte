<script lang="ts">
  ...
  function getTermTheme(): any {
    ...
    return termThemes[palette] || termThemes.dark
  }
  ...
  onMount(() => {
    ...
    // Watch for long URLs
    term.onData(() => {}) // no-op, we use onRender instead
    // Actually, intercept writes to detect URLs
    const origWrite = term.write.bind(term)
    // We'll check in onmessage instead
